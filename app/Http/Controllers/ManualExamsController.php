...
class ManualExamsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $this->ensureTable();
        $this->ensureStudentsTableStatusAndSection();
            ...
            if ($selectedStudentId) {
                ...
                if (Schema::hasTable('student_behavior_attributes')) {
                    $row = DB::table('student_behavior_attributes')
                        ->where('student_id', $selectedStudentId)
                        ->where('class_id', $selectedClassId)
                        ->when(!empty($selectedSection), function ($q) use ($selectedSection) {
                            $q->where('section', $selectedSection);
                        })
                        ->when(!empty($sessionValue), function ($q) use ($sessionValue) {
                            $q->where('session', $sessionValue);
                        })
                        ->where('term', $term)
                        ->first();
                    if ($row) {
                        ...
                        if (is_array($decoded)) {
                            $behaviorAttributesFromTable = $decoded;
                        }
        ...
        if (!empty($selectedClassId) && isset($students) && $students && $students->count() > 0) {
            $studentIds = $students->pluck('grno')->map(function ($v) {
                return (int)$v;
            })->all();
            ...
            if (Schema::hasTable('student_behavior_attributes') && !empty($studentIds)) {
                $rows = DB::table('student_behavior_attributes')
                    ...
                    ->get();
                foreach ($rows as $r) {
                    $behaviorRows[(int)$r->student_id] = $r;
                }
            }
            ...
            foreach ($studentData as $studentGrno => $marks) {
                ...
                if (!empty($marks)) {
                    ...
                    foreach ($marks as $k => $v) {
                        if (is_string($v) && strtolower(trim($v)) === 'a') {
                            $marks[$k] = 'A';
                        }
                    }
    ...
    public function principalRemarks(Request $request)
    {
        ...
        $studentsMap = [];
        if (!empty($studentIds)) {
            $st = DB::table('students')->when($tenantId, function ($q) use ($tenantId) {
                $q->where('tenant_id', $tenantId);
            })->whereIn('grno', $studentIds);
            if (!empty($selectedSection)) $st->where('section', $selectedSection);
            foreach ($st->get() as $s) {
                $studentsMap[(int)$s->grno] = $s;
            }
        }
    ...
    public function printAll(Request $request)
    {
        ...
        if (Schema::hasTable('manual_exam_entries') && $selectedClassId) {
            ...
            $existingEntries = DB::table('manual_exam_entries')
                ...
                ->where('term', $term)
                ->where(function ($q) {
                    $q->where('subject', 'all')->orWhereNull('subject')->orWhere('subject', '');
                })
                ->get();
            foreach ($existingEntries as $row) {
                $existingByStudentId[$row->student_id] = json_decode($row->data, true) ?: [];
    ...
    public function printEntry(Request $request)
    {
        ...
        // Fix: Pass extra data needed for filters
        $classesForTeacher = [];
        if (auth()->check()) {
            if (auth()->user()->role === 'teacher') {
                $teacherId = DB::table('teachers')->where('email', auth()->user()->email)->value('id');
                $classesForTeacher = DB::table('classes')->where('teacher_id', $teacherId)->get();
            } else {
                $classesForTeacher = DB::table('classes')
                    ->when($tenantId, function($q) use ($tenantId) {
                        $q->where('tenant_id', $tenantId);
        ...
        $allSections = ['A', 'B', 'C', 'D', 'Blue', 'Green', 'Yellow', 'Pink'];
        $availableSections = [];
        if ($selectedClassId) {
        ...
        try {
            ...
            foreach ($csvData as $row) {
                ...
                foreach ($marksData as $mk => $mv) {
                    if (is_string($mv) && strtolower(trim($mv)) === 'a') {
                        $marksData[$mk] = 'A';
                    }
                }
    ...
    private function ensureExamTermsTable(): void
    {
        ...
        if ($existingTerms === 0) {
            DB::table('exam_terms')->insert([
                ...
                [
                    'term_name' => 'December Test',
                    'display_name' => 'December Test',
                    'description' => 'December test',
                    'applicable_classes' => json_encode(['5', '6', '7', '8']),
                    'sort_order' => 4,
                    'is_active' => true,
                    'created_at' => now(),
    ...
    /**
     * Ensure students table has status and section columns
     */
    private function ensureStudentsTableStatusAndSection(): void
    {
        if (Schema::hasTable('students')) {
            if (!Schema::hasColumn('students', 'section') || !Schema::hasColumn('students', 'status')) {
                Schema::table('students', function (Blueprint $table) {
                    if (!Schema::hasColumn('students', 'section')) {
                        $table->string('section')->nullable()->after('class_id');
                    }
                    if (!Schema::hasColumn('students', 'status')) {
                        $table->string('status')->default('active')->after('grno');
                    }
                });
            }
        }
