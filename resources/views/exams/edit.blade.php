        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">Edit Exam Details</h5>
                </div>
                    ...
                    <form action="{{ route('exams.update', $exam->id) }}" method="POST">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control @error('exam_date') is-invalid @enderror" 
                                       id="exam_date" name="exam_date" value="{{ old('exam_date', $exam->exam_date ? $exam->exam_date->format('Y-m-d') : '') }}" 
                                       required>
                                @error('exam_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="time" class="form-control @error('exam_time') is-invalid @enderror" 
                                       id="exam_time" name="exam_time" value="{{ old('exam_time', $exam->exam_time ? $exam->exam_time->format('H:i') : '') }}" required>
                                @error('exam_time')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                ...
                                <textarea class="form-control @error('instructions') is-invalid @enderror" 
                                          ...
                                          placeholder="Enter exam instructions...">{{ old('instructions', $exam->instructions) }}</textarea>
                                @error('instructions')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-12">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="auto_submit" name="auto_submit" value="1" 
                                           {{ old('auto_submit', $exam->auto_submit ?? 1) ? 'checked' : '' }}>
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="show_results" name="show_results" value="1" 
                                           {{ old('show_results', $exam->show_results ?? 0) ? 'checked' : '' }}>
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="randomize_questions" name="randomize_questions" value="1" 
                                           {{ old('randomize_questions', $exam->randomize_questions ?? 0) ? 'checked' : '' }}>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <a href="{{ route('exams.show', $exam->id) }}" class="btn btn-info me-2">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="alert alert-info">
                        ...
                        <p class="mb-2">
                            Status: <span class="badge bg-{{ $exam->status == 'published' ? 'success' : ($exam->status == 'draft' ? 'warning' : 'secondary') }}">
                                {{ ucfirst($exam->status) }}
                            </span>
                        </p>
                        <p class="mb-2">Total Questions: <strong>{{ $exam->questions()->count() }}</strong></p>
                        <p class="mb-0">Created: <strong>{{ $exam->created_at->format('M d, Y') }}</strong></p>
                    </div>
                    ...
                    <div class="alert alert-warning">
                        ...
                        <ul class="mb-0 small">
                            @if($exam->status == 'published')
                                <li>This exam is currently published</li>
                                <li>Students can see and attempt this exam</li>
                                <li>Be careful when making changes</li>
                            @elseif($exam->status == 'draft')
                                <li>This exam is in draft mode</li>
                                <li>Students cannot see this exam yet</li>
                                <li>Add questions before publishing</li>
                            @endif
                            <li>Passing marks should be reasonable</li>
                            <li>Test all settings before publishing</li>
                        </ul>
                    </div>

                    @if($exam->questions()->count() > 0)
                        <div class="alert alert-success">
                            <h6><i class="bi bi-check-circle"></i> Questions Added</h6>
                            <p class="mb-0 small">This exam has {{ $exam->questions()->count() }} questions and is ready to be published.</p>
                        </div>
                    @else
                        <div class="alert alert-danger">
                            <h6><i class="bi bi-x-circle"></i> No Questions</h6>
                            <p class="mb-0 small">You need to add questions before this exam can be published to students.</p>
                        </div>
                    @endif
                </div>
            </div>
