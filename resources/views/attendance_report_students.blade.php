    ...
    <div class="col-lg-12">
        <div class="card border-0 shadow-sm overflow-hidden rounded-4">
            <div class="card-header bg-primary text-white py-3 d-flex justify-content-between align-items-center border-0">
                <h5 class="card-title p-0 m-0 text-white fw-bold"><i class="bi bi-calendar3 me-2"></i>Student Monthly Report</h5>
                <div class="badge bg-white text-primary fs-6 py-2 px-3 rounded-pill shadow-sm">
                    {{ date('F', mktime(0, 0, 0, $month, 10)) }} {{ $year }}
                </div>
            </div>
            <div class="card-body pt-4">
                <form method="GET" action="{{ route('attendance.reports.students') }}" class="row g-3 mb-4 p-3 bg-light rounded-4 mx-0 align-items-end border border-white">
                    <div class="col-md-3">
                        <label class="form-label small fw-bold text-muted text-uppercase ls-1">Select Class</label>
                        <select name="class_id" class="form-select border-0 shadow-sm rounded-3 py-2" onchange="this.form.submit()">
                                    ...
                                    <tr class="attendance-row">
                                        <td class="sticky-column bg-white fw-bold text-dark border-right" style="left: 0; z-index: 50; border-right: 2px solid #f8f9fa;">
                                            <div class="d-flex align-items-center py-1">
                                                ...
                                                <div class="lh-1">
                                                    <div class="text-truncate" style="max-width: 150px;">{{ $s->studentName }}</div>
                                                    <small class="text-muted fw-normal" style="font-size: 0.65rem;">Roll: {{ $s->rollNo }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        @for($d=1; $d<=$daysInMonth; $d++)
                                            @php 
                                                $st = $attendance[$s->id][$d] ?? ''; 
                ...
                @else
                    <div class="text-center py-5">
                        <img src="{{ asset('dashra/img/not-found.svg') }}" alt="Not Found" style="width: 150px; opacity: 0.5;">
                        <p class="mt-3 text-muted">Select a class to view the attendance report.</p>
                    </div>
                @endif
            </div>
        </div>
